...
The bibliography now uses explicit in-text citations instead of blanket `\nocite{*}`. All 15 BibTeX entries are mapped to concrete chapter text. Three legal references had broken gov.cn news URLs and were corrected to the National People's Congress legal database. No obvious hallucinated reference remains in the current set.
